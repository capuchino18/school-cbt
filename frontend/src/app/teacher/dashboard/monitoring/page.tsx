...
export default function TeacherDashboard() {
  ...
  // Real-time Countdown Timer untuk Sesi Aktif di Dashboard
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      examSessions.forEach((ses) => {
        ...
        if (diff <= 0) {
          newTimeLefts[ses.id] = 'Waktu Selesai (Berakhir)';
        } else {
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <header className="flex justify-between items-center bg-slate-900/90 backdrop-blur-xl border border-slate-800 p-6 rounded-3xl shadow-2xl">
            ...
            <div className="flex items-center space-x-3">
              <Link href="/teacher/exams" className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-2xl shadow transition">
                + Buat & Input Soal Ujian
              </Link>
          ...
          <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-800 p-6 rounded-3xl shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-4">
              <div>
                <h2 className="text-sm font-bold text-white">Daftar Sesi Ujian Aktif / Berlangsung ({examSessions.length})</h2>
                <p className="text-xs text-slate-400">Pantau waktu ujian dan akses live monitoring siswa secara real-time</p>
              </div>
              <button onClick={fetchPublishedSessions} className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold rounded-xl border border-slate-700">
                Refresh Data
              </button>
            </div>
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {examSessions.length === 0 ? (
                    <tr><td colSpan={7} className="p-8 text-center text-slate-500">Belum ada sesi ujian aktif. Buat dan terbitkan soal melalui menu &quot;Buat & Input Soal Ujian&quot;.</td></tr>
                  ) : (
                    examSessions.map((s) => {
                      const timeLeft = timeLefts[s.id] || 'Menghitung...';
                      const isEnded = timeLeft.includes('Waktu Selesai');
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-800 p-6 rounded-3xl shadow-xl space-y-3">
              ...
              <p className="text-xs text-slate-400">Buat soal pilihan ganda, essay, atau hybrid, tentukan bobot penilaian, dan simpan sebagai draf atau terbitkan kapan saja.</p>
